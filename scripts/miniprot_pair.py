...
def build_contig_lengths(ref_fasta: str) -> Dict[str, int]:
    ...
    if SeqIO is None:
        raise RuntimeError("Biopython not installed; install biopython or omit --ref-fasta.")
    contig_len = {}
...
def main():
    ...
    ap.add_argument("--ref-fasta", default=None, help="Reference FASTA (to clamp coordinates).")
    ap.add_argument(
        "--chrom-sizes",
        default=None,
    ...
    # Pair N/C terminals
    print(f"Pairing N/C terminals (distance: {args.min_distance}-{args.max_distance} bp)...")
    paired_genes, unpaired_hits = pair_nc_terminals(hits, args.min_distance, args.max_distance)
